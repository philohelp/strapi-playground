...
const RightLinksCompo = () => {
  ...
  const [prompt2, setPrompt2] = useState(undefined);
  ...
  const handlePromptSubmit = () => {
    if (model && prompt && temperature && maxTokens) {
      ...
      const completePrompt = `${prompt}${prompt2}`;
      completionAPI
        .create({ model, prompt: completePrompt, temperature, maxTokens })
        .then((data) => {
          console.log(data);
                    ...
                    <TabPanels>
                      <TabPanel>
                        ...
                        >
                          <Textarea
                            placeholder={formatMessage({
                              ...
                              defaultMessage:
                                "Si besoin, donnez des éléments de contexte",
                            })}
                            label="Context"
                            name="content"
                        ...
                        >
                          <Textarea
                            placeholder={formatMessage({
                              id: getTrad("Modal.tabs.prompt2.placeholder"),
                              defaultMessage:
                                "Expliquez précisément ce que vous désirez obtenir",
                            })}
                            label="Prompt"
                            name="content"
